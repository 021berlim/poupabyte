 ...
 return (
  ...
  >
   ...
   <div
    ...
    className={cn(
     'fixed inset-y-0 z-10 hidden h-dvh max-h-dvh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex',
     side === 'left'
...
function SidebarMenuSkeleton({
 ...
 const width = React.useMemo(() => {
  const hash = Array.from(widthId).reduce((acc, char) => acc + char.charCodeAt(0), 0)
  return `${(hash % 40) + 50}%`
 }, [widthId])
